...
UPLOAD_FOLDER = "C:/Users/vinee/wound/data/Medetec_foot_ulcer_224/test/images"
UPLOAD_FOLDER1 = "C:/Users/vinee/wound/data1/src"
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['UPLOAD_FOLDER1'] = UPLOAD_FOLDER1
UPLOAD_FOLDER2 = "C:/Users/vinee/wound/data1/src"
...
def predict():
    ...
    if request.method == "POST":
        ...
        filename = "foot-ulcer-0027.png"
        # filename1 = "lena.jpg"
        image.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        # image.save(os.path.join(app.config['UPLOAD_FOLDER1'],filename1))
        img5 = cv2.imread('C:/Users/vinee/wound/data/Medetec_foot_ulcer_224/test/images/foot-ulcer-0027.png')
        cv2.imwrite('C:/Users/vinee/wound/data1/src/lena.jpg', img5)
        ...
        class BilinearUpSampling2D(Layer):
            ...
            def call(self, x, mask=None):
                if self.target_size is not None:
                    return resize_images_bilinear(x, target_height=self.target_size[0],
            ...
            for x in range(0, patch_size_x):
                for y in range(0, patch_size_y):
                    for z in range(0, patch_size_z):
                        if (x + x_ >= 0 and x + x_ < d.shape[0] and y + y_ >= 0 and y + y_ < d.shape[
                            1] and z + z_ >= 0 and z + z_ < d.shape[2]):
        ...
        def generateFullPatchsCentered(d, patch_size_x, patch_size_y, patch_size_z):
            patch_nb = int(
                2 * (d.shape[0] / patch_size_x) * 2 * (d.shape[1] / patch_size_y) * 2 * (d.shape[2] / patch_size_z))
        ...
        def generatorRandomPatchs(features, labels, batch_size, patch_size_x, patch_size_y, patch_size_z):
            batch_features = np.zeros((batch_size, patch_size_x, patch_size_y, patch_size_z, features.shape[4]),
                                      dtype='float16')
            batch_labels = np.zeros((batch_size, patch_size_x, patch_size_y, patch_size_z, labels.shape[4]),
                                    dtype='float16')
        ...
        def generatorRandomPatchsLabelCentered(features, labels, batch_size, patch_size_x, patch_size_y, patch_size_z):
            ...
            while True:
                for i in range(batch_size):
                    ...
                    batch_features[i] = extractPatch(features[id], patch_size_x, patch_size_y, patch_size_z, x, y, z)
                    batch_labels[i] = extractPatch(labels[id], patch_centered_size_x, patch_centered_size_y,
                                                   patch_centered_size_z,
                                                   int(x + patch_size_x / 4), int(y + patch_size_y / 4),
                                                   int(z + patch_size_z / 4))
        ...
        def generatorRandomPatchsDolz(features, labels, batch_size, patch_size_x, patch_size_y, patch_size_z):
            ...
            batch_labels = np.zeros(
                (batch_size, int(patch_size_x / 2) * int(patch_size_y / 2) * int(patch_size_z / 2), 2),
                dtype=labels.dtype)
            ...
            while True:
                for i in range(batch_size):
                    ...
                    batch_features[i] = extractPatch(features[id], patch_size_x, patch_size_y, patch_size_z, x, y, z)
                    tmpPatch = extractPatch(labels[id], int(patch_size_x / 2), int(patch_size_y / 2),
                                            int(patch_size_z / 2),
                                            int(x + patch_size_x / 4), int(y + patch_size_y / 4),
                                            int(z + patch_size_z / 4))
        ...
        def generatorRandomPatchsLinear(features, labels, patch_features_x, patch_features_y, patch_features_z,
            ...
            patch_features = np.zeros((1, patch_features_x, patch_features_y, patch_features_z, features.shape[4]),
                                      dtype=features.dtype)
            patch_labels = np.zeros((1, patch_labels_x, patch_labels_y, patch_labels_z, labels.shape[4]),
                                    dtype=labels.dtype)

            if (
                    patch_features_x % patch_labels_x != 0 or patch_features_y % patch_labels_y != 0 or patch_features_z % patch_labels_z != 0):
                sys.exit(0x00F0)

            if (
                    patch_features_x < patch_labels_x or patch_features_y < patch_labels_y or patch_features_z < patch_labels_z):
                sys.exit(0x00F1)
            ...
            while True:
                ...
                patch_features[0] = extractPatch(rot_features, patch_features_x, patch_features_y, patch_features_z, x,
                                                 y, z)
        ...
        def randomPatchsAugmented(in_dataset, gd_dataset, patch_number, patch_in_size, patch_gd_size):
            patchs_in = np.zeros((patch_number, patch_in_size[0], patch_in_size[1], patch_in_size[2]),
                                 dtype=in_dataset.dtype)
            patchs_gd = np.zeros((patch_number, patch_gd_size[0], patch_gd_size[1], patch_gd_size[2]),
                                 dtype=gd_dataset.dtype)

            if (patch_in_size[0] % patch_gd_size[0] != 0 or patch_in_size[1] % patch_gd_size[1] != 0 or patch_in_size[
                2] % patch_gd_size[2] != 0):
            ...
            for count in range(patch_number):
                ...
                patchs_in[count] = extractPatch(in_dataset[id], patch_in_size[0], patch_in_size[1], patch_in_size[2], x,
                                                y, z)
                patchs_gd[count] = extractPatch(gd_dataset[id], patch_gd_size[0], patch_gd_size[1], patch_gd_size[2],
                                                x + mx - sx, y + my - sy, z + mz - sz)
            ...
            for x in range(0, image.shape[0], patchs.shape[1]):
                for y in range(0, image.shape[1], patchs.shape[2]):
                    for z in range(0, image.shape[2], patchs.shape[3]):
                        image[x:x + patchs.shape[1], y:y + patchs.shape[2], z:z + patchs.shape[3]] = patchs[i, :, :, :,
                                                                                                     0]
                        i = i + 1
            return image
        ...
        def FCN_Vgg16_16s(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=1):
            ...
            # Block 1
            x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv1', kernel_regularizer='l2')(
                img_input)
            ...
            x = Conv2D(classes, (1, 1), kernel_initializer='he_normal', activation='linear', padding='valid',
                       strides=(1, 1), kernel_regularizer='l2')(x)
        ...
        class SegNet:
            ...
            def get_SegNet(self):
                ...
                decoder_conv6 = Conv2D(self.n_filters, kernel_size=7, activation='relu', padding='same')(
                    UpSampling2D(size=(2, 2))(conv5))
                decoder_conv7 = Conv2D(self.n_filters, kernel_size=5, activation='relu', padding='same')(
                    UpSampling2D(size=(2, 2))(decoder_conv6))
                decoder_conv8 = Conv2D(self.n_filters, kernel_size=5, activation='relu', padding='same')(
                    UpSampling2D(size=(2, 2))(decoder_conv7))
                # decoder_conv9 = Conv2D(self.n_filters, kernel_size=5, activation='relu', padding='same')(UpSampling2D(size=(2, 2))(decoder_conv8))
                decoder_conv9 = Conv2D(1, kernel_size=1, activation='sigmoid', padding='same')(
                    UpSampling2D(size=(2, 2))(decoder_conv8))
        ...
        class Unet2D:
            ...
            def get_unet_model_5_levels(self):
                ...
                up6 = Conv2D(self.n_filters * 16, 2, activation='relu', padding='same')(
            ...
            def get_unet_model_4_levels(self):
                ...
                up5 = Conv2D(self.n_filters * 16, 2, activation='relu', padding='same')(
            ...
            def get_unet_model_yuanqing(self):
                ...
                up9 = Conv2D(int(self.n_filters / 2), 2, activation='relu', padding='same')(
        ...
        for image_batch, label_batch in data_gen.generate_data(batch_size=len(x_test), test=True):
            ...
            save_results(prediction, 'rgb', "C:/Users/vinee/wound/wnd/static/data/", test_label_filenames_list)
            img1 = cv2.imread('C:/Users/vinee/wound/wnd/static/data/foot-ulcer-0027.png')
            cv2.imwrite('C:/Users/vinee/wound/data1/src/horse_r.jpg', img1)
        ...
        src = np.array(Image.open('C:/Users/vinee/wound/data1/src/lena.jpg'))
        mask = np.array(
            Image.open('C:/Users/vinee/wound/data1/src/horse_r.jpg').resize(src.shape[1::-1], Image.BILINEAR))
        ...
        Image.fromarray(dst.astype(np.uint8)).save('C:/Users/vinee/wound/data1/dst/numpy_image_mask.jpg')

        # ![](data1/dst/numpy_image_mask.jpg)

        mask = np.array(Image.open('C:/Users/vinee/wound/data1/src/horse_r.jpg').convert('L').resize(src.shape[1::-1],
                                                                                                     Image.BILINEAR))
        ...
        from PIL import Image
        import cv2
        img1 = cv2.imread('C:/Users/vinee/wound/data1/src/horse_r.jpg')
        dest_not1 = cv2.bitwise_not(img1, mask=None)
        cv2.imwrite('C:/Users/vinee/wound/data1/src/horse1.jpg', dest_not1)
        import numpy as np
        from PIL import Image

        src = np.array(Image.open('C:/Users/vinee/wound/data1/src/lena.jpg'))
        mask = np.array(
            Image.open('C:/Users/vinee/wound/data1/src/horse1.jpg').resize(src.shape[1::-1], Image.BILINEAR))
        ...
        Image.fromarray(dst.astype(np.uint8)).save('C:/Users/vinee/wound/data1/dst/numpy_image_mask2.jpg')

        # ![](data1/dst/numpy_image_mask.jpg)

        mask = np.array(Image.open('C:/Users/vinee/wound/data1/src/horse1.jpg').convert('L').resize(src.shape[1::-1],
                                                                                                    Image.BILINEAR))
        ...
        masked_part = cv2.imread('C:/Users/vinee/wound/data1/dst/numpy_image_mask.jpg')
        unmasked_part = cv2.imread('C:/Users/vinee/wound/data1/dst/numpy_image_mask2.jpg')
        d = correlate(masked_part, unmasked_part)
        if d == 0.00000000000000:
            st = "0"
        else:
            st = "1"
        data = Wound(uid=uid, val=d, dat=str(date.today()), st=st)
        db.session.add(data)
        db.session.commit()

        return render_template("predict.html", p=filename, d1=d)
...
def user_index(id):
    # filelist = glob.glob("C:/Users/Rajasree/Desktop/wound/data/Medetec_foot_ulcer_224/test/images/*.*")
    # for filePath in filelist:
    #     try:
    #         os.remove(filePath)
    #     except:
    #         print("Error while deleting file")
    # filelist1 = glob.glob("C:/Users/Rajasree/Desktop/wound/wnd/static/data/*.*")
...
def reset():
    filelist = glob.glob("C:/Users/vinee/wound/data/Medetec_foot_ulcer_224/test/images/*.*")
    for filePath in filelist:
        try:
            os.remove(filePath)
        except:
            print("Error while deleting file")
    filelist1 = glob.glob("C:/Users/vinee/wound/wnd/static/data/*.*")
    for filePath in filelist1:
        try:
            os.remove(filePath)
        except:
            print("Error while deleting file")
    filelist2 = glob.glob("C:/Users/vinee/wound/data1/src/*.*")
    ...
    filelist3 = glob.glob("C:/Users/vinee/wound/data1/dst/*.*")
...
def analysis(id):
    d = Wound.query.filter_by(uid=id).all()
    for i in d:
        if i.st == "0":
            delet = Wound.query.filter_by(uid=id).all()

            try:
                db.session.delete(delet)
                # db.session.delete(d)
                db.session.commit()
                return redirect('/analysis/' + str(current_user.id))
            except:
                return 'There was a problem deleting that task'

    return render_template("analysis.html", d=d)
...
def user_register():
    if request.method == 'POST':
        ...
        my_data = Login(name=name, username=email, height=height, weight=weight, age=age, contact=contact,
